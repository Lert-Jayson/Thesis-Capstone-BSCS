...
class SoilLayer:
	def __init__(self, all_sprites):
		...
		# graphics
		self.soil_surfs = import_folder_dict('graphics/lupa/soil/')
		...
		self.any_tile_hit = False

	


	def create_soil_grid(self):
		ground = pygame.image.load('data/mapa.png')
		h_tiles, v_tiles = ground.get_width() // TILE_SIZE, ground.get_height() // TILE_SIZE
		
		self.grid = [[[] for col in range(h_tiles)] for row in range(v_tiles)]
		for x, y, _ in load_pygame('data/tmx/1stmap.tmx').get_layer_by_name('farmable').tiles():
