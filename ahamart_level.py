...
from player_minimarket2 import *
...
class Level_ahamart:
    ...
    def setup(self):
        tmx_data = load_pygame('data/tmx/interior_mini_market2.tmx')
        ...
        for obj in tmx_data.get_layer_by_name('player'):
            if obj.name == 'start':
                self.player = Player_minimarket2(pos = (obj.x,obj.y), 
                    group = self.all_sprites,
                    collision_sprites = self.collision_sprites,
                    interaction = self.interaction_sprites,
                    display_surface= self.display_surface)
            if obj.name == 'ahamart_entrance_exit':
                Interaction((obj.x,obj.y), (obj.width,obj.height), self.interaction_sprites, obj.name)
        Generic(
            pos = (0,0),
            surf = pygame.image.load('graphics/interior_mini_market2.png').convert_alpha(),
            groups = self.all_sprites,
            z = LAYERS['ground'])
    def indicator(self):
        collided_interaction_sprite = pygame.sprite.spritecollide(self.player,self.interaction_sprites,False)
        if collided_interaction_sprite:
            if collided_interaction_sprite[0].name == 'ahamart_entrance_exit':
