...
class Overlay_cave:
	def __init__(self,player):
		...
		self.hud = pygame.image.load('graphics/overlay/cave_gui.png').convert_alpha()
...
##########################################################
		...
		if self.player.had_gem:
			gem_surf = pygame.image.load('graphics/overlay/gem.png').convert_alpha()
			gem_rect = gem_surf.get_rect(center = OVERLAY_POSITIONS['gem'])
			self.display_surface.blit(gem_surf, gem_rect)

			numG = self.player.item_inventory['gem']
			numG_surf = self.font.render(str(numG) , False, 'white')
			numG_rect = numG_surf.get_rect(midbottom = (SCREEN_WIDTH - 190, SCREEN_HEIGHT - 25))
			self.display_surface.blit(numG_surf,numG_rect)

		if self.player.had_essence:
			essence_surf = pygame.image.load('graphics/overlay/essence.png').convert_alpha()
			essence_rect = essence_surf.get_rect(center = OVERLAY_POSITIONS['essence'])
			self.display_surface.blit(essence_surf, essence_rect)

			numE = self.player.item_inventory['essence']
			numE_surf = self.font.render(str(numE) , False, 'white')
			numE_rect = numE_surf.get_rect(midbottom = (SCREEN_WIDTH - 70, SCREEN_HEIGHT - 25))
			self.display_surface.blit(numE_surf,numE_rect)

		if self.player.had_key:
			key_surf = pygame.image.load('graphics/overlay/key.png').convert_alpha()
			key_rect = key_surf.get_rect(center = OVERLAY_POSITIONS['keys'])
			self.display_surface.blit(key_surf, key_rect)

			numk = self.player.item_inventory['keys']
			numk_surf = self.font.render(str(numk) , False, 'white')
			numk_rect = numk_surf.get_rect(midbottom = (SCREEN_WIDTH - 70, SCREEN_HEIGHT - 150))
			self.display_surface.blit(numk_surf,numk_rect)
